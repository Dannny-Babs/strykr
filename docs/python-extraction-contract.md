...
The existing research/NLP pipeline remains independent from compliance decisions. Product-facing Python output must be JSON matching `dealersync-extraction-v1` and is ingested through `PythonExtractionAdapter` rather than imported directly by UI code.
