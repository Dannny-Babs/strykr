...
const existingOrganization = db.select({ id: organizations.id }).from(organizations).limit(1).get();
if (existingOrganization) {
  console.log("Seed skipped: the database already contains product data. Use the explicit reset-db command only when a destructive local reset is intended.");
  process.exit(0);
}
...
db.transaction((transaction) => {
  transaction.insert(organizations).values([
    ...
    { id: "org-system", name: "DealerSync Administration", type: "COMPLIANCE_FIRM", status: "ACTIVE", createdAt: now, updatedAt: now },
  ]).run();
  ...
  transaction.insert(dealerships).values(dealerNames.map((name, index) => ({ id: `dealer-${index + 1}`, organizationId: "org-northfield-group", legalName: `${name} Holdings Inc.`, tradeName: name, registrationNumber: `ON-${String(41023 + index * 97).padStart(6, "0")}`, address: `${100 + index} Main Street`, city: ["Hamilton", "Mississauga", "Toronto", "Ottawa", "London", "Kitchener"][index % 6], province: "ON", postalCode: "M5V 2T6", contactName: "Compliance Office", contactEmail: `compliance${index + 1}@example.test`, contactPhone: "416-555-0100", status: "ACTIVE", createdAt: now, updatedAt: now }))).run();
  transaction.insert(users).values([
    { id: "user-regulator", organizationId: "org-regulator", dealershipId: null, name: "Julia Mercer", email: "reviewer@example.test", role: "REGULATOR_REVIEWER", status: "ACTIVE", createdAt: now, updatedAt: now },
    { id: "user-dealer-admin", organizationId: "org-northfield-group", dealershipId: "dealer-1", name: "Jordan Smith", email: "dealer.admin@example.test", role: "DEALER_ADMIN", status: "ACTIVE", createdAt: now, updatedAt: now },
    { id: "user-dealer-compliance", organizationId: "org-northfield-group", dealershipId: "dealer-1", name: "Priya Shah", email: "dealer.user@example.test", role: "DEALER_USER", status: "ACTIVE", createdAt: now, updatedAt: now },
    { id: "user-system-admin", organizationId: "org-system", dealershipId: null, name: "System Administrator", email: "admin@example.test", role: "SYSTEM_ADMIN", status: "ACTIVE", createdAt: now, updatedAt: now },
  ]).run();
  transaction.insert(reportingPeriods).values(dealerNames.map((_, index) => ({ id: `period-${index + 1}-2025`, dealershipId: `dealer-${index + 1}`, name: "2025 Annual Transaction Register", startDate: "2025-01-01", endDate: "2025-12-31", status: "OPEN", createdAt: now, closedAt: null }))).run();
  transaction.insert(feeSchedules).values({ id: "fee-on-2025", amount: 22, effectiveFrom: "2025-01-01", effectiveTo: null, jurisdiction: "ON", createdAt: now }).run();
  const rules = [
  ...
  transaction.insert(reconciliationRules).values(rules.map(([code, name, description, severity, category]) => ({ id: `rule-${code.toLowerCase()}`, code, name, description, severity, category, enabled: true, version: "ruleset-v1", configuration: code === "DATE001" ? JSON.stringify({ toleranceDays: 7 }) : "{}" }))).run();
  transaction.insert(importBatches).values({ id: "batch-seed-transactions", organizationId: "org-northfield-group", dealershipId: "dealer-1", reportingPeriodId: "period-1-2025", sourceType: "TRANSACTION_REGISTER", fileName: "northfield-2025-register.csv", status: "COMPLETED", totalRows: 2000, validRows: 2000, warningRows: 0, rejectedRows: 0, duplicateRows: 0, createdBy: "user-dealer-admin", createdAt: now, completedAt: now }).run();
  ...
  for (const group of chunks(vehicleRows)) transaction.insert(vehicles).values(group).run();
  ...
  for (const group of chunks(transactionRows, 30)) transaction.insert(transactions).values(group).run();
  ...
  for (const group of chunks(registrationRows, 35)) transaction.insert(registrationRecords).values(group).run();
  transaction.insert(audits).values({ id: "audit-northfield-2025", dealershipId: "dealer-1", reportingPeriodId: "period-1-2025", name: "Northfield 2025 transaction review", scope: "Annual transaction register reconciliation", status: "RECONCILIATION", assignedReviewer: "user-regulator", startedAt: "2026-08-01T13:00:00.000Z", dueAt: "2026-09-15T21:00:00.000Z", completedAt: null, createdAt: now, updatedAt: now }).run();
});

const result = runReconciliation({ dealershipId: "dealer-1", reportingPeriodId: "period-1-2025", actor: { id: "user-system-admin", organizationId: "org-system", dealershipId: null, role: "SYSTEM_ADMIN" } });
