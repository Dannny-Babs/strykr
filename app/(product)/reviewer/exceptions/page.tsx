...
export default async function ReviewerExceptionsPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; status?: string; impact?: string; sort?: string }> }) {
  const actor = await requireProductActor(["REGULATOR_REVIEWER"]); const params = await searchParams; const data = await getExceptionsPage(actor, { page: Number(params.page ?? 1), query: params.q, status: params.status, impact: params.impact, sort: params.sort, reviewer: true });
