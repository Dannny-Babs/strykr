...
export default async function DealerTransactionsPage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string; state?: string; sort?: string }> }) {
  const actor = await requireProductActor(["DEALER_ADMIN", "DEALER_USER"]); const params = await searchParams; const data = await getDealerTransactionsPage(actor, { page: Number(params.page ?? 1), query: params.q, state: params.state, sort: params.sort });
