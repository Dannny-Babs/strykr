...
export async function POST(request: Request, context: { params: Promise<{ auditId: string }> }) {
  try { const actor = await getActor(); const { auditId } = await context.params; return Response.json(await createAuditFinding({ auditId, ...requestSchema.parse(await request.json()), actor }), { status: 201 }); }
