...
export async function GET(request: Request) {
  try {
    ...
    const ids = normalizeSelectedIds((new URL(request.url).searchParams.get("ids") ?? "").split(","), 50); const records = getAuthorizedTransactionsByIds(actor, ids);
    ...
    return new Response(rows.map((row) => row.map(csvCell).join(",")).join("\n"), { headers: { "content-type": "text/csv; charset=utf-8", "content-disposition": 'attachment; filename="dealersync-selected-transactions.csv"' } });
