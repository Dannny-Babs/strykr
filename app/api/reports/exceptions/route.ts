...
export async function GET(request: Request) {
  try {
    ...
    return new Response(rows.map((row) => row.map(csvCell).join(",")).join("\n"), { headers: { "content-type": "text/csv; charset=utf-8", "content-disposition": 'attachment; filename="cordena-exception-review.csv"' } });
