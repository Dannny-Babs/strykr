...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    ...
    >
      <table
        data-slot="table"
        className={cn("w-full caption-bottom text-[13px] leading-4 text-secondary-foreground", className)}
        {...props}
      />
    </div>
  )
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "h-[52px] px-4 py-2 align-middle whitespace-nowrap [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
