...
export function MetricRow({ items }: { items: Array<{ label: string; value: string; note?: string; tone?: "default" | "warning" | "danger"; href?: string }> }) {
  ...
  return <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Key metrics">{items.map((item, index) => { const body = <><div className="flex items-start justify-between gap-3"><p className="text-xs font-medium text-muted-foreground">{item.label}</p>{item.tone && item.tone !== "default" ? <Badge variant={item.tone === "danger" ? "destructive" : "warning"}><CircleAlert className="size-3" />{item.tone === "danger" ? "Attention" : "Review"}</Badge> : null}</div><strong className="mt-4 block text-3xl font-semibold leading-9 tracking-[-0.035em] text-foreground tabular-nums">{item.value}</strong>{item.note ? <p className="mt-1 text-xs text-muted-foreground">{item.note}</p> : null}</>; const className = cn("min-h-36 rounded-[14px] border p-5", palette[index % palette.length]); return item.href ? <Link key={item.label} href={item.href} className={cn(className, "group block transition-[border-color,background-color] duration-150 hover:border-primary-border hover:bg-primary-soft active:scale-[0.99]")} aria-label={`${item.label}: ${item.value}. Open related records`}>{body}<ArrowUpRight className="absolute size-0" /></Link> : <div key={item.label} className={className}>{body}</div>; })}</section>;
}
