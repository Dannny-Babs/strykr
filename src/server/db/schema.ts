...
export const users = pgTable("users", {
  id: text("id").primaryKey(), organizationId: text("organization_id").notNull().references(() => organizations.id), dealershipId: text("dealership_id").references(() => dealerships.id), authProviderId: text("auth_provider_id"), name: text("name").notNull(), email: text("email").notNull(), emailVerifiedAt: text("email_verified_at"), passwordHash: text("password_hash"), role: text("role").notNull(), status: text("status").notNull().default("ACTIVE"), onboardingData: text("onboarding_data").notNull().default("{}"), onboardingCompletedAt: text("onboarding_completed_at"), lastSignedInAt: text("last_signed_in_at"), ...timestamps,
}, (table) => [uniqueIndex("user_email_idx").on(table.email), uniqueIndex("user_auth_provider_idx").on(table.authProviderId), index("user_dealership_idx").on(table.dealershipId)]);

// Legacy tables are intentionally retained for rollback safety. No application path writes them.
...
export const authTokens = pgTable("auth_tokens", {
  id: text("id").primaryKey(), userId: text("user_id").notNull().references(() => users.id), purpose: text("purpose").notNull(), tokenHash: text("token_hash").notNull(), expiresAt: text("expires_at").notNull(), consumedAt: text("consumed_at"), createdAt: text("created_at").notNull(),
}, (table) => [uniqueIndex("auth_token_hash_idx").on(table.tokenHash), index("auth_token_user_purpose_idx").on(table.userId, table.purpose), index("auth_token_expiry_idx").on(table.expiresAt)]);

export const rateLimitBuckets = pgTable("rate_limit_buckets", {
  key: text("key").primaryKey(), count: integer("count").notNull().default(0), resetAt: text("reset_at").notNull(), updatedAt: text("updated_at").notNull(),
...
export const listingObservations = pgTable("listing_observations", {
  id: text("id").primaryKey(), dealershipId: text("dealership_id").notNull().references(() => dealerships.id), vehicleId: text("vehicle_id").references(() => vehicles.id), vin: text("vin").notNull(), normalizedVin: text("normalized_vin").notNull(), source: text("source").notNull(), sourceUrl: text("source_url").notNull(), observedAt: text("observed_at").notNull(), listingStatus: text("listing_status").notNull(), price: doublePrecision("price"), extractionMethod: text("extraction_method").notNull(), confidence: doublePrecision("confidence"), createdAt: text("created_at").notNull(),
...
export const reconciliationRules = pgTable("reconciliation_rules", {
  id: text("id").primaryKey(), code: text("code").notNull(), name: text("name").notNull(), description: text("description").notNull(), severity: text("severity").notNull(), category: text("category").notNull(), enabled: boolean("enabled").notNull().default(true), version: text("version").notNull(), configuration: text("configuration").notNull().default("{}"),
}, (table) => [uniqueIndex("rule_code_version_idx").on(table.code, table.version)]);

export const feeSchedules = pgTable("fee_schedules", {
  id: text("id").primaryKey(), amount: doublePrecision("amount").notNull(), effectiveFrom: text("effective_from").notNull(), effectiveTo: text("effective_to"), jurisdiction: text("jurisdiction").notNull(), createdAt: text("created_at").notNull(),
